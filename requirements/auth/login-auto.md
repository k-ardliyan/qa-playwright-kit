# REQ-AUTH-AUTO: Login — auto — Target App

<!--
  Catalog AUTH_CHALLENGE_MODE=auto. Setup wizard menulis requirements/login.md dari mode yang dipilih.
...
- **AC-08:** Setelah password, tantangan auto diselesaikan manusia; skenario ditandai (@manual) karena OTP/CAPTCHA tidak diotomasi di pipeline (AUTH_CHALLENGE_MODE hanya untuk auth:setup).
...
- **AC-15:** Mengakses halaman protected tanpa sesi mengarahkan pengguna ke halaman login tanpa menampilkan konten protected (deep-link protection).
- **AC-16:** Setelah login sukses, reload halaman tidak mengakhiri sesi — pengguna tetap login di `/dashboard`.
- **AC-17:** Navigasi back browser setelah login tidak mengakhiri sesi; akses ulang area sukses tidak meminta login ulang.
- **AC-18:** Klik ganda pada tombol submit tidak memproses otentikasi dua kali (tombol disabled atau menampilkan loading selama proses).
- **AC-19:** Identifier berisi markup/script dirender sebagai teks (di-escape), tidak dieksekusi, dan submit ditolak.
- **AC-20:** Logout mengakhiri sesi; akses halaman protected setelah logout diarahkan kembali ke `/login`.
...
### SC-07: Verifikasi OTP atau CAPTCHA (auto-detect) (@manual)
...
**Prekondisi:** Pengguna di `https://app.example.com/login`, kredensial valid, AUTH_CHALLENGE_MODE=auto.

**Input Data:**

- identifier: credential:user.email
- password: credential:user.password
- challengeMode: literal:auto
...
3. Isi field password (`password`, `pass`, atau `kata sandi`)
4. Klik tombol submit (`Masuk`, `Login`, `Sign in`, atau `Log in`)
5. Selesaikan OTP atau CAPTCHA yang muncul (mode auto: CAPTCHA di browser; OTP di browser jika headed, fallback terminal jika TTY)

**Hasil yang Diharapkan:**

- Setelah tantangan selesai, URL pathname mengandung `/dashboard` **DAN TIDAK** mengandung `/login`
- Form login tidak terlihat lagi
- OTP/CAPTCHA tidak diotomasi di pipeline — skenario tetap (@manual). AUTH_CHALLENGE_MODE=auto hanya membantu `npm run auth:setup` mendeteksi tantangan lalu pause di browser atau terminal.
...
- AUTH_CHALLENGE_MODE=auto — bantu sesi via `npm run auth:setup` / `auth:setup:headed`; skenario tantangan tetap (@manual)
