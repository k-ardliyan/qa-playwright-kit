# REQ-AUTH-NONE: Login — none — Target App

<!--
  Catalog AUTH_CHALLENGE_MODE=none. Setup wizard menulis requirements/login.md dari mode yang dipilih.
...
- **AC-08:** Form login menyediakan tombol/icon toggle show/hide password yang mengubah atribut type input antara password dan text.
- **AC-09:** Sistem secara otomatis memotong (trim) karakter spasi di awal dan akhir identifier pada saat submit sehingga login dengan kredensial valid tetap berhasil.
- **AC-10:** Form login dapat di-submit menggunakan penekanan tombol keyboard Enter ketika fokus berada pada input field.
...
### SC-07: Login Berhasil dengan Kredensial Valid (@success)
...
**Prekondisi:** Akun `TEST_USER_EMAIL` (atau USERNAME/PHONE) terdaftar di aplikasi (lihat `config/environments/{APP_ENV}.env` — JANGAN tulis nilainya di sini).
...
- URL pathname mengandung `/dashboard` **DAN TIDAK** mengandung `/login`
- Form login tidak terlihat lagi (sudah diganti konten dashboard/beranda)
- Session tersimpan di `.auth/{APP_ENV}/user.json` (atau `.auth/{APP_ENV}/user.json`) via auth.setup
