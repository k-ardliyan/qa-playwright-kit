...
17. **Content assert fail** (`assertPdfContains` missing needles): re-read **scenario** Expected Result / Input Data / Hasil yang Diharapkan for the correct tokens; optionally call MCP `extract_pdf_text` for actual text; fix needles only if the plan was mis-transcribed — **do not** replace with canned fields (judul/kode/nama/invoice schema or demo tokens like `QA-KIT-SAMPLE-PDF`).
...
| `assertPdfContains` fails with partial text present                               | Needles not from scenario, or mis-transcribed                                            | Re-read plan Expected Result / requirement Hasil; call `extract_pdf_text` for actual dump; align needles to **scenario tokens only** — never swap in a canned field set or demo tokens (`QA-KIT-SAMPLE-PDF`) |
