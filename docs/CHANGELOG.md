...
- **Bug nyata ditemukan & diperbaiki — satuan campur di tabel report:** `testsGenerated` memakai `generatedFiles.length` (jumlah **file**) dan dicetak bersebelahan dengan `testsPassing` (jumlah **test**), menghasilkan tabel seperti "Tests Generated 1 | Tests Passing 18" — requirement 19 skenario tampak menghasilkan satu test. Kini `testCount`/`testsGenerated` dihitung dari isi spec via `extractTestMetadataFromSpec` (termasuk `test.skip`, karena test yang di-skip tetap tergenerate), di tiga titik: resume generator eksternal, handoff `awaiting-generator`, dan ringkasan report. Deskripsi manifest "Number of tests generated" akhirnya benar. **Verifikasi:** test regresi gagal pada kode lama (Expected 3, Received 1) dan lolos pada perbaikan; 104 test integration hijau.
- **Klaim self-healing diselaraskan dengan realita:** README menyatakan framework memperbaiki test sendiri ("Locator berubah? Framework memperbaiki sendiri"). Tidak ada kode runtime yang menyunting spec saat gagal (`auto-heal`/`applyFix`/`repairTest` = nol hit). Healing dilakukan oleh AI healer agent lewat MCP (`get_test_failures`, `record_ai_note`), sementara runtime hanya menegakkan routing kegagalan (`feedback-router` + loop terbatas). Kalimat kini menyebut itu alih-alih menjanjikan perbaikan otomatis.
- **Dead code dihapus (592 LOC, 5 file):** `src/shared/mcp/{locator-generator-adapter,locator-priority,semantic-verifier}.ts` dan `src/shared/utils/{cli-runner,factories}.ts` — nol importer di produksi maupun test, tanpa barrel export, tanpa penyebutan di npm scripts/CI/playwright.config/docs/skills. **Sengaja dipertahankan** (terverifikasi direferensikan, bukan dead): `tools/mcp/src/index-mcp.ts` + `index.ts` (package `main`/`bin`/`start:http`), modul `src/shared/evidence` + `src/observability` (diimpor test), dan `tools/scripts/{smart-shard,failed-only}.ts` (kebijakan repo: utilitas maintainer/CI keep-by-design). **Verifikasi:** `tsc` bersih setelah penghapusan.
