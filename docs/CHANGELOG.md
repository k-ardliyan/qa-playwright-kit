...
- **Visual baseline di clone bersih:** test visual demo gagal dengan "A snapshot doesn't exist" karena baseline PNG bersifat platform-specific dan di-gitignore. Spec demo kini mem-bootstrap baseline sekali (halaman `setContent` tetap, tidak ada UI nyata yang bisa ter-regresi), lalu membandingkan sungguhan — mencoba `updateSnapshots: 'missing'` di level project tidak dipakai karena itu bukan opsi project di Playwright 1.63 dan Playwright menandai `shouldNotRetryTest` sehingga retry tidak menolong.
